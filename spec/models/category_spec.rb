...
RSpec.describe Category, type: :model do
  ...
  describe 'validations' do
    it 'is valid with valid attributes' do
      category = Category.new(name: 'Food', icon: 'food.jpg', user: user)
      expect(category).to be_valid
    end

    it 'is invalid without a name' do
      category = Category.new(icon: 'food.jpg', user: user)
      expect(category).to be_invalid
      expect(category.errors[:name]).to include("can't be blank")
    end

    it 'is invalid with a name shorter than 3 characters' do
      category = Category.new(name: 'AB', icon: 'food.jpg', user: user)
      expect(category).to be_invalid
    ...
    it 'is invalid with a name longer than 30 characters' do
      category = Category.new(name: 'A' * 31, icon: 'food.jpg', user: user)
      expect(category).to be_invalid
